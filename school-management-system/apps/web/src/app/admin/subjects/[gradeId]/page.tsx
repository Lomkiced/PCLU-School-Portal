    ...
    const fetchData = async () => {
        ...
        try {
            const [gradeRes, subjectsRes] = await Promise.all([
                ...
                api.get(`/subjects?gradeLevelId=${gradeId}`)
            ]);
            setGradeLevel(gradeRes.data.data);
            setSubjects(subjectsRes.data.data || []);
        } catch (error) {
            console.error("Failed to fetch data:", error);
            // If grade level doesn't exist, redirect back
    ...
    return (
        <div className="space-y-6">
            ...
            <nav className="flex items-center gap-2 text-sm text-[hsl(var(--muted-foreground))]">
                ...
                <Link href="/admin/subjects" className="hover:text-[hsl(var(--foreground))] transition-colors">
                    Subjects
                </Link>
                <ChevronRight className="w-3.5 h-3.5" />
                <span className="text-[hsl(var(--foreground))] font-semibold">{gradeLevel.name}</span>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-3">
                    <Link
                        href="/admin/subjects"
                        className="p-2 rounded-xl bg-[hsl(var(--muted))] hover:bg-[hsl(var(--muted)/0.8)] transition-colors"
                    >
                        <ArrowLeft className="w-5 h-5" />
                    </Link>
                    <div>
                        <h2 className="text-xl font-bold flex items-center gap-2">
                            <BookOpen className="w-5 h-5 text-[hsl(var(--primary))]" /> {gradeLevel.name} Curriculum
                        </h2>
                        <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
                            {subjects.length} subject{subjects.length !== 1 ? "s" : ""} taught in this grade level
                        </p>
                    </div>
                </div>
                <button
                            ...
                            {paginated.length === 0 && (
                                <tr>
                                    <td colSpan={8} className="text-center py-12 text-[hsl(var(--muted-foreground))]">
                                        ...
                                        <p className="font-medium">{search || typeFilter ? "No subjects match your filters" : "No subjects found for this grade level."}</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
