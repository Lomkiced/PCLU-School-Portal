...
export function CreateDepartmentModal({ open, onClose, onSuccess }: CreateDepartmentModalProps) {
    ...
    const handleClose = () => {
        if (!submitting) {
            setName(""); setNameError(""); setApiError("");
            onClose();
        }
    };
            ...
            <div className="relative bg-[hsl(var(--card))] rounded-2xl w-full max-w-md shadow-2xl border border-[hsl(var(--border))]">
                <div className="flex items-center justify-between px-6 py-4 border-b border-[hsl(var(--border))]">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-bold text-base">Create Department</h3>
                            <p className="text-xs text-[hsl(var(--muted-foreground))]">Add a new department to the system</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
                    ...
                    <div className="space-y-1.5">
                        <label className="text-xs font-semibold text-[hsl(var(--muted-foreground))]">Department Name *</label>
                        <input
                            className={inputClass(!!nameError)}
                            placeholder="e.g. Science Department, Math Department"
                            value={name}
                ...
                <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-[hsl(var(--border))]">
                    ...
                    <button onClick={handleSubmit} disabled={submitting} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[hsl(var(--primary))] text-white text-sm font-semibold hover:bg-[hsl(var(--primary-hover))] transition-all shadow-md shadow-[hsl(var(--primary)/0.25)] disabled:opacity-60">
                        {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Creating...</> : <><Plus className="w-4 h-4" /> Create Department</>}
                    </button>
                </div>
            </div>
        </div>
