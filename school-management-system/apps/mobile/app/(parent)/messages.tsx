...
const conversations = [
    { id: "1", name: "Prof. Garcia (Adviser)", message: "Maria is doing great!", time: "1h", unread: 1 },
    { id: "2", name: "Admin Office", message: "Payment reminder sent.", time: "3h", unread: 0 },
    { id: "3", name: "Parents Group", message: "PTA meeting this Saturday.", time: "1d", unread: 3 },
];
            ...
            <FlatList data={conversations} keyExtractor={(i) => i.id} contentContainerStyle={{ paddingHorizontal: 20 }}
                renderItem={({ item }) => (
                    <TouchableOpacity style={styles.conv} activeOpacity={0.7}>
                        <View style={[styles.avatar, { backgroundColor: "#8b5cf618" }]}><Text style={[styles.avatarText, { color: "#8b5cf6" }]}>{item.name[0]}</Text></View>
                        <View style={{ flex: 1 }}>
                        ...
                        {item.unread > 0 && <View style={[styles.badge, { backgroundColor: "#8b5cf6" }]}><Text style={styles.badgeText}>{item.unread}</Text></View>}
                    </TouchableOpacity>
                )}
            />
        </SafeAreaView>
